...
int heuristica_manhattan (tEstado *e)
{
    ...
    for (int i = 0 ; i < N ; i++)
    {
        for (int j = 0 ; j < N ; j++)
        {
            if (e->celdas[i][j] != puzle_final[i][j] && e->celdas[i][j] != 0)
            {
                int k = 0, encontrado = 0;
                while (!encontrado && k < N)
                {
                    int l = 0;
                    while (!encontrado && l < N)
                    {
                        if (e->celdas[i][j] == puzle_final[k][l]){
                            h += abs(i-k) + abs(j-l);
                            encontrado = 1;
                        }
...
int busqueda_voraz ()
{
    int objetivo=0, visitados=0, max=0, nodos_generados=0, aux = 0;
    tNodo *Actual=(tNodo*) calloc(1,sizeof(tNodo));
    tNodo *Inicial=nodoInicial();
    ...
    while (!esVacia(Abiertos) && !objetivo)
    {
        ...
        if (!objetivo && !buscaRepe(Actual,Cerrados)) {
            Sucesores = expandir(Actual);
            nodos_generados += longitudLista(Sucesores);
            Abiertos=ordenarLista(Abiertos,Sucesores,0);
            InsertarUltimo(&Cerrados, Actual, sizeof(tNodo));
            aux = longitudLista(Abiertos);
        }
        if (aux > max)
            max = aux;
    ...
    if (objetivo)
        dispSolucion(Actual);
    printf("\nVisitados= %d\n", visitados);
    printf("Maxima longitud de abiertos= %d\n", max);
